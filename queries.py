...
PROGRAMMING_LANGUAGE_HAS_USE_QUERY = """
   SELECT DISTINCT ?has_use_label WHERE {
        SERVICE <https://query.wikidata.org/sparql> {

          <%programming_language_subject%> wdt:P366 ?has_use.
          ?has_use rdfs:label ?has_use_label.
  
          FILTER(LANG(?has_use_label) = "en")
        }
    }
    LIMIT 10 
"""
